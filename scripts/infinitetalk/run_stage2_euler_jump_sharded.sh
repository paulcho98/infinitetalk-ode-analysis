...
# between ~7 h and ~90 min, which is why scripts/infinitetalk/run_stage2a_metrics_sharded.sh exists for the
...
#
# Usage:
#   bash scripts/infinitetalk/run_stage2_euler_jump_sharded.sh            # all 7 cells, NS=5 -> 35 procs
#   NS=10 bash scripts/infinitetalk/run_stage2_euler_jump_sharded.sh      # 70 procs (heavier, faster)
#   CELLS="euler_on_on euler_on_nocfg" bash scripts/infinitetalk/...      # subset (e.g. cells that finished early)
set -uo pipefail
...
for c in "${READY[@]}"; do
    ...
    CUDA_VISIBLE_DEVICES=${GPU_ARR[$gpu]} "$PY" scripts/infinitetalk/eval_ode_perceptual_v2_infinitetalk.py \
