...
ANALYSIS_ROOT = Path(os.environ.get("ODE_ANALYSIS_ROOT_OMNI", "/home/work/.local/ode_analysis")) / "14B_mouthweight"
TRAJ_DIR = Path(os.environ.get("ODE_TRAJ_ROOT_OMNI", "/home/work/.local/ode_full_trajectories")) / "14B_mouthweight" / "cfg4.5"

VARIANTS = {
    "trajectory_cfg45": {
        "path": Path("perceptual_v2") / "metrics.csv",
    ...
    "trajectory_nocfg": {
        "path": Path("trajectory_nocfg") / "metrics.csv",
        "label": "50-step noCFG",
        "color": "black",
        "linestyle": "--",
        "linewidth": 2.5,
    ...
    "euler_cfg45_cfg45": {
        "path": Path("euler_cfg45_cfg45") / "metrics.csv",
        "label": "Euler CFG->CFG",
        "color": "tab:blue",
        "linestyle": "-",
        "linewidth": 1.6,
    ...
    "euler_nocfg_cfg45": {
        "path": Path("euler_nocfg_cfg45") / "metrics.csv",
    ...
    "euler_nocfg_nocfg": {
        "path": Path("euler_nocfg_nocfg") / "metrics.csv",
    ...
    "euler_cfg45_nocfg": {
        "path": Path("euler_cfg45_nocfg") / "metrics.csv",
...
CFG_FRONTIER = {
    ...
    "CFG=3.0": {
        "path": Path("trajectory_cfg3.0") / "metrics.csv",
        "cfg": 3.0,
        "color": "tab:purple",
    },
    "CFG=4.5": {
    ...
    "CFG=6.0": {
        "path": Path("trajectory_cfg6.0") / "metrics.csv",
        "cfg": 6.0,
        "color": "tab:brown",
    },
}
...
def main() -> None:
    ...
    for cfg in VARIANTS.values():
        cfg["path"] = args.analysis_root / cfg["path"]
    for cfg in CFG_FRONTIER.values():
        cfg["path"] = args.analysis_root / cfg["path"]
